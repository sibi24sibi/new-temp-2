...
const PaymentModal = ({ isOpen, onClose, paymentUrl, selectedPlan, formatPrice, isAnnual }) => {
        ...
        <div className="w-full fixed inset-0 bg-black bg-opacity-50 flex flex-row items-center justify-center z-50">
            <div className="relative w-[80%]  min-h-[80vh] bg-transparent rounded-lg flex flex-row gap-4">
                ...
                <div className="bg-white rounded-lg p-6 shadow-lg">
                    <h3 className="text-xl font-bold text-emerald-600 mb-4">Selected Plan</h3>
                    <div className="plan-card max-w-md mx-auto">
                        <h2 className="plan-name">{selectedPlan.name}</h2>
                        {selectedPlan.price ? formatPrice(selectedPlan.price) :
                            isAnnual ?
            ...
            async (position) => {
                try {
                    ...
                    if (supportedCurrencies.includes(currency)) {
                        ...
                        console.log('Currency not supported, falling back to USD:', currency);
                        setDefaultCurrency();
                    }
                } catch (error) {
    ...
    const pricingData = {
        ...
        expert: [
            ...
            },
            {
                ...
                price: 'Custom',
                features: [
                    'If you want Additional dashboards'
                ]
            }
        ]
    };
                ...
                {pricingData[activeKit].map((plan, index) => (
                    <div className="plan-card" key={index}>
                        ...
                        {plan.price === 'Custom' ? (
                            ...
                            <button onClick={() => { handleBuyClick(plan); console.log(plan) }} className="buy-button">
                                Buy Now
                            </button>
                        )}
                    </div>
